...
class Mailing(models.Model):
    ...
    periodicity = models.CharField(
        choices=PERIODICITY_DATA, verbose_name="периодичность"
    )
    check_periodicity = models.IntegerField(
        default="0", verbose_name="дней до следующей отправки"
    )
    status = models.CharField(
        choices=STATUS_DATA, default="создана", verbose_name="статус", **NULLABLE
    )
    message = models.ForeignKey(
        MailingMessage, on_delete=models.SET_NULL, verbose_name="сообщение", **NULLABLE
    )
